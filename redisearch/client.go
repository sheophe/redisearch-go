...
func (i *Client) indexWithDefinition(indexName string, schema *Schema, definition *IndexDefinition) (err error) {
	...
	_, err = i.conn.Do("FT.CREATE", args...)
	return
}

// AddField Adds a new field to the index.
func (i *Client) AddField(indexName string, f Field) error {
	args := redis.Args{indexName}
	...
	_, err = i.conn.Do("FT.ALTER", args...)
	return err
}

// Index indexes a list of documents with the default options
func (i *Client) Index(indexName string, docs ...Document) error {
	return i.IndexOptions(indexName, DefaultIndexingOptions, docs...)
}
...
func (i *Client) Search(indexName string, q *Query) (docs []Document, total int, err error) {
	args := redis.Args{indexName}
	args = append(args, q.serialize()...)

	res, err := redis.Values(i.conn.Do("FT.SEARCH", args...))
	if err != nil {
		return
	}
...
func (i *Client) AliasAdd(indexName, name string) (err error) {

	args := redis.Args{name}.Add(indexName)
	_, err = redis.String(i.conn.Do("FT.ALIASADD", args...))
	return
}
...
func (i *Client) AliasDel(name string) (err error) {
	...
	_, err = redis.String(i.conn.Do("FT.ALIASDEL", args...))
	return
}
...
func (i *Client) AliasUpdate(indexName, name string) (err error) {

	args := redis.Args{name}.Add(indexName)
	_, err = redis.String(i.conn.Do("FT.ALIASUPDATE", args...))
	return
}
...
func (i *Client) DictAdd(dictionaryName string, terms []string) (newTerms int, err error) {
	...
	newTerms, err = redis.Int(i.conn.Do("FT.DICTADD", args...))
	return
}
...
func (i *Client) DictDel(dictionaryName string, terms []string) (deletedTerms int, err error) {
	...
	deletedTerms = 0
	args := redis.Args{dictionaryName}.AddFlat(terms)
	deletedTerms, err = redis.Int(i.conn.Do("FT.DICTDEL", args...))
	return
}
...
func (i *Client) DictDump(dictionaryName string) (terms []string, err error) {
	...
	terms, err = redis.Strings(i.conn.Do("FT.DICTDUMP", args...))
	return
}

// SpellCheck performs spelling correction on a query, returning suggestions for misspelled terms,
// the total number of results, or an error if something went wrong
func (i *Client) SpellCheck(indexName string, q *Query, s *SpellCheckOptions) (suggs []MisspelledTerm, total int, err error) {

	args := redis.Args{indexName}
	args = append(args, q.serialize()...)
	args = append(args, s.serialize()...)

	res, err := redis.Values(i.conn.Do("FT.SPELLCHECK", args...))
	if err != nil {
		return
	}
	total = 0
...
// Aggregate
func (i *Client) Aggregate(indexName string, q *AggregateQuery) (aggregateReply [][]string, total int, err error) {
	...
	if !validCursor {
		args := redis.Args{indexName}
		args = append(args, q.Serialize()...)
		res, err = redis.Values(i.conn.Do("FT.AGGREGATE", args...))
	} else {
		args := redis.Args{"READ", indexName, q.Cursor.Id}
		res, err = redis.Values(i.conn.Do("FT.CURSOR", args...))
	}
	if err != nil {
		return
	}
...
func (i *Client) Get(indexName string, docId string) (doc *Document, err error) {
	doc = nil

	var reply interface{}
	args := redis.Args{indexName, docId}
	reply, err = i.conn.Do("FT.GET", args...)
	if reply != nil {
...
func (i *Client) MultiGet(indexName string, documentIds []string) (docs []*Document, err error) {
	docs = make([]*Document, len(documentIds))

	var reply interface{}
	args := redis.Args{indexName}.AddFlat(documentIds)
	reply, err = i.conn.Do("FT.MGET", args...)
	if reply != nil {
		var array_reply []interface{}
		array_reply, err = redis.Values(reply, err)
		if err != nil {
...
func (i *Client) Explain(indexName string, q *Query) (string, error) {

	args := redis.Args{indexName}
	args = append(args, q.serialize()...)

	return redis.String(i.conn.Do("FT.EXPLAIN", args...))
}

// Drop deletes the index and all the keys associated with it.
func (i *Client) Drop(indexName string) error {

	_, err := i.conn.Do("FT.DROP", indexName)
	return err
}
...
func (i *Client) DropIndex(indexName string, deleteDocuments bool) error {

	var err error = nil
	if deleteDocuments {
		_, err = i.conn.Do("FT.DROPINDEX", indexName, "DD")
	} else {
		_, err = i.conn.Do("FT.DROPINDEX", indexName)
	}
	return err
}
...
func (i *Client) Delete(indexName string, docId string, deleteDocument bool) (err error) {
	return i.delDoc(indexName, docId, deleteDocument)
}

// DeleteDocument delete the document from the index and also delete the HASH key in which the document is stored
func (i *Client) DeleteDocument(indexName string, docId string) (err error) {
	return i.delDoc(indexName, docId, true)
}

// Internal method to be used by Delete() and DeleteDocument()
func (i *Client) delDoc(indexName string, docId string, deleteDocument bool) (err error) {

	if deleteDocument {
		_, err = i.conn.Do("FT.DEL", indexName, docId, "DD")
	} else {
		_, err = i.conn.Do("FT.DEL", indexName, docId)
	}
	return
}
...
// index exists
func (i *Client) Info(indexName string) (*IndexInfo, error) {

	res, err := redis.Values(i.conn.Do("FT.INFO", indexName))
	if err != nil {
		return nil, err
	}
...
func (i *Client) SetConfig(option string, value string) (string, error) {
	...
	return redis.String(i.conn.Do("FT.CONFIG", args...))
}
...
func (i *Client) GetConfig(option string) (map[string]string, error) {
	...
	values, err := redis.Values(i.conn.Do("FT.CONFIG", args...))
	if err != nil {
		return nil, err
	}
...
func (i *Client) GetTagVals(index string, filedName string) ([]string, error) {
	...
	return redis.Strings(i.conn.Do("FT.TAGVALS", args...))
}
...
func (i *Client) SynAdd(indexName string, terms []string) (int64, error) {
	...
	return redis.Int64(i.conn.Do("FT.SYNADD", args...))
}
...
func (i *Client) SynUpdate(indexName string, synonymGroupId int64, terms []string) (string, error) {
	...
	return redis.String(i.conn.Do("FT.SYNUPDATE", args...))
}
...
func (i *Client) SynDump(indexName string) (map[string][]int64, error) {
	...
	args := redis.Args{indexName}
	values, err := redis.Values(i.conn.Do("FT.SYNDUMP", args...))
	if err != nil {
		return nil, err
	}
...
func (i *Client) AddHash(indexName string, docId string, score float32, language string, replace bool) (string, error) {

	args := redis.Args{indexName, docId, score}
	if language != "" {
	...
	return redis.String(i.conn.Do("FT.ADDHASH", args...))
}

// Returns a list of all existing indexes.
func (i *Client) List() ([]string, error) {

	res, err := redis.Values(i.conn.Do("FT._LIST"))
	if err != nil {
		return nil, err
	}
